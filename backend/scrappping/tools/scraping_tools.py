...
def _standardise_job_record(raw: dict, platform: str) -> dict:
    ...
    """
    source_url = raw.get("source_url") or raw.get("detail_url") or ""
...
async def _rate_limit(min_delay: Optional[float] = None, max_delay: Optional[float] = None):
...
def _build_indeed_search_url(keyword: str, location: str = "", job_type: str = "",
                             ...
                             days_filter: Optional[int] = None) -> str:
...
async def scrape_indeed_jobs(
    ...
    all_listings: list[dict] = []   # collected across search pages
    ...
    search_crawler_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS,
        page_timeout=60000,
        wait_until="load",
        session_id="indeed_search_session",
        magic=True,
        simulate_user=True,
    ...
    # ── Phase 1: Scrape search pages with UndetectedAdapter (bypasses Cloudflare) ──
    try:
        ...
        async with AsyncWebCrawler(
            ...
            while page_num < max_pages:
                ...
                logger.info(f"Indeed search page {page_num + 1}: {url}")

                result = await _fetch_with_retry(search_crawler, url, search_crawler_config)
                if not result or not result.html:
                    logger.warning(f"Failed to fetch Indeed page {page_num + 1}")
                    break
                ...
                for l in new:
                    seen_urls.add(l["detail_url"])
                all_listings.extend(new)
                logger.info(f"Collected {len(new)} new listings from search page {page_num + 1}")
        ...
        logger.error(f"Indeed search-page phase failed: {exc}")
        if not all_listings:
            raise

    logger.info(f"Phase 1 complete: {len(all_listings)} listings collected from search pages")

    if not all_listings:
        return all_jobs

    detail_proxy = proxy_rotator.next() or proxy
    detail_browser_kwargs = {
        "headless": True,
        "enable_stealth": True,
        "browser_type": "chromium",
        "user_agent": ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    ...
    if detail_proxy:
        detail_browser_kwargs["proxy"] = detail_proxy
    detail_browser_config = BrowserConfig(**detail_browser_kwargs)
    detail_crawler_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS,
        page_timeout=45000,
        wait_until="domcontentloaded",
        session_id="indeed_detail_session",
        magic=True,
        simulate_user=True,
        override_navigator=True,
    ...
    try:
        async with AsyncWebCrawler(config=detail_browser_config) as detail_crawler:
            ...
            async def fetch_and_parse_indeed_detail(listing_ref):
                ...
                @retry_with_backoff()
                async def fetch_op():
                    res = await detail_crawler.arun(url=url_to_crawl, config=detail_crawler_config)
        ...
        logger.error(f"Indeed detail-page phase failed: {exc}")
        # If we got listings but detail fetch failed, standardise them without details
        if not all_jobs and all_listings:
            logger.info("Falling back: standardising listings without detail-page data")
            for listing in all_listings:
                listing["source_url"] = listing.get("detail_url", "")
                listing["apply_link"] = listing.get("detail_url", "")
                try:
                    all_jobs.append(_standardise_job_record(listing, "indeed"))
                except Exception as std_exc:
                    logger.error(f"Error standardising fallback: {std_exc}")
