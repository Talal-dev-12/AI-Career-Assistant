...
from mabd.models.models import User, Job, InterviewSession
from mabd.services.llm import generate_interview_questions, evaluate_interview_transcript
from fastapi import HTTPException

def start_interview_session(session: Session, user_id: str, job_id: str) -> InterviewSession:
...
def submit_interview_response(session: Session, session_id: str, question_index: int, answer: str) -> InterviewSession:
    # 1. Fetch session
...
def evaluate_interview_session(session: Session, session_id: str) -> InterviewSession:
    # 1. Fetch session
    db_session = session.get(InterviewSession, session_id)
    if not db_session:
        raise HTTPException(status_code=404, detail=f"Interview session {session_id} not found")
