...
export default function OnboardingPage() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && userId) {
      ...
      setUploadLogs(["Uploading CV file to backend gateway..."]);
      ...
      try {
        ...
        const interval = setInterval(async () => {
          try {
            const logsRes = await fetch(`${apiUrl}/users/${userId}/pipeline/logs`);
            if (logsRes.ok) {
              const logsData = await logsRes.json();
              if (logsData.length > 0) {
                const latestLog = logsData[logsData.length - 1];
                setUploadProgress(latestLog.progress || 10);
                setUploadLogs(logsData.map((l: { message: string }) => l.message));

                if (latestLog.progress === 100 || latestLog.stage === "completed" || latestLog.stage === "error") {
                  clearInterval(interval);
                  setIsUploading(false);

                  // Prefill step 3 from parsed CV
                  const profileRes = await fetch(`${apiUrl}/users/${userId}/profile`);
                  if (profileRes.ok) {
                    const profileData = await profileRes.json();
                    if (profileData.education && profileData.education.length > 0) {
                      setDegree(profileData.education[0].degree || "Bachelor of Computer Science");
                      setInstitution(profileData.education[0].institution || "DHA Suffa University");
                    }
                  }
                }
              }
        ...
        console.error("Resume pipeline trigger failed:", err);
        setUploadLogs((prev) => [...prev, "Error: Upload or pipeline trigger failed."]);
        setIsUploading(false);
      }
    }
  };
  ...
  const completeOnboarding = async () => {
    ...
    const nameToSave = currentName && currentName !== "Google User" ? currentName : "John Doe";
    localStorage.setItem("userName", nameToSave);

    // Save/Sync to backend
    if (userId) {
      try {
        // Fetch current profile first to preserve parsed skills/experience
        const profileRes = await fetch(`${apiUrl}/users/${userId}/profile`);
        let existingSkills: string[] = [];
        let existingExp: any[] = [];
        if (profileRes.ok) {
          const existingProfile = await profileRes.json();
          existingSkills = existingProfile.skills || [];
          existingExp = existingProfile.experience || [];
        }
        ...
        await fetch(`${apiUrl}/users/${userId}/profile`, {
          ...
          body: JSON.stringify({
            ...
            skills: existingSkills.length > 0 ? existingSkills : ["React", "JavaScript", "HTML", "CSS"],
            experience: existingExp,
            education: [{ degree, institution, year: "2025" }],
            locations: [country],
            target_roles: [targetJob],
            profile_version: 1,
          }),
        });
      } catch (err) {
        console.error("Failed to save final onboarding profile:", err);
      }
    }
        ...
        {step === 2 && (
          <div className={styles.formBody}>
            ...
            {!isUploading ? (
              ...
              <div className={profileStyles.progressCard} style={{ background: "rgba(255,255,255,0.02)", borderRadius: 12, border: "1px solid var(--border-color)", padding: 20 }}>
                <div className={profileStyles.progressHeader}>
                  <span style={{ fontSize: 13, fontWeight: 600 }}>CV Processing & Job Harvesting Pipeline...</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "var(--primary)" }}>{uploadProgress}%</span>
                </div>
                <div className={profileStyles.barBack} style={{ margin: "12px 0" }}>
                  <div className={profileStyles.barFill} style={{ width: `${uploadProgress}%` }}></div>
                </div>
                <div style={{ marginTop: 12, maxHeight: 150, overflowY: "auto", fontSize: 11, color: "var(--text-muted)", textAlign: "left", display: "flex", flexDirection: "column", gap: 6, border: "1px solid rgba(255,255,255,0.05)", borderRadius: 8, padding: 10, background: "rgba(0,0,0,0.2)" }}>
                  {uploadLogs.map((logStr, idx) => (
