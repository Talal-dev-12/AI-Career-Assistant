...
export default function ProfilePage() {
  ...
  const processFile = async (file: File) => {
    ...
    setUploadLogs(["[system] Uploading CV file to backend gateway..."]);
        ...
        try {
          ...
          if (logsRes.ok) {
            ...
            if (logsData.length > 0) {
              ...
              setUploadLogs(logsData.map((l: { message: string }) => `[agent] ${l.message}`));
          ...
          console.error("Error polling logs on profile page:", err);
        }
      }, 1000);
    } catch (err) {
      console.error(err);
      setUploadLogs((prev) => [...prev, "[system] [ERROR] Failed to upload or parse CV."]);
      setTimeout(() => {
        setIsUploading(false);
      }, 3000);
    }
