...
function JobsContent() {
  ...
  const handleScrapeTrigger = async () => {
    setIsScraping(true);
    try {
      const res = await fetch(`${apiUrl}/jobs/scrape`, {
        method: "POST",
      });
      if (res.ok) {
        const data = await res.json();
        alert(`Job scraper finished!\nSource: ${data.source}\nTotal Fetched: ${data.fetched}\nNew Saved to DB: ${data.newly_saved}`);
        await fetchJobs();
      }
    } catch (err) {
      console.error(err);
      alert("Error triggering live job scraper.");
    }
    setIsScraping(false);
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.listPane}>
        <div className="glass searchBar" style={{ padding: 16 }}>
          <div className={styles.searchInputs} style={{ marginBottom: 12 }}>
          ...
          >
            <Sparkles size={14} />
            <span>{isScraping ? "Scraping Live Boards..." : "Scrape & Verify Live Jobs"}</span>
          </button>
        </div>
